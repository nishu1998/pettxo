...
class _PostActionSheet extends StatelessWidget {
  ...
  const _PostActionSheet({required this.actions});
              ...
              child: Column(
                ...
                children: [
                  for (var index = 0; index < actions.length; index++) ...[
                    _PostActionRow(action: actions[index]),
...
class _PostActionRow extends StatelessWidget {
  ...
  const _PostActionRow({required this.action});

  @override
  Widget build(BuildContext context) {
    final theme = Theme.of(context);
    final activeColor = action.isDestructive
